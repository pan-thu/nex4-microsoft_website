# WhatWeDo Card Redesign — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the current 4-column bordered grid with a 2×2 large photo card layout where each card reveals its description and CTA on hover.

**Architecture:** Single file rewrite of `src/components/home/WhatWeDo.tsx`. No new files, no new dependencies. Pure CSS transitions via Tailwind group/hover utilities; Framer Motion kept only for scroll-in entrance animation.

**Tech Stack:** React 19, TypeScript, Tailwind CSS 4.1, Framer Motion 12, React Router 7, Lucide React

---

### Task 1: Rewrite WhatWeDo.tsx

**Files:**
- Modify: `src/components/home/WhatWeDo.tsx` (full rewrite)

**Image assignments (from `public/images/`):**
| Service | Image | Rationale |
|---|---|---|
| Workplace Security | `card-bg-2.png` | Purple wave — evokes protection/energy |
| Workplace AI | `card-bg-4.png` | Blue 3D cubes — evokes data/intelligence |
| Workplace Automation | `card-bg-1.png` | Rainbow wave — evokes motion/flow |
| Cloud Migration | `card-bg-3.png` | Architectural form — evokes structure/scale |
...
│                                          │
│  ─────────────────────────────────────  │
│  01 ─ WORKPLACE SECURITY       ← eyebrow │
│  Workplace Security            ← h3 32px │
│                                          │
...
**Rest state:**
- Image at `scale-100`, dark overlay `bg-gradient-to-t from-black/80 via-black/40 to-transparent`
- Eyebrow + title always visible at bottom
- Description: `opacity-0 translate-y-3`
- CTA: `opacity-0 translate-y-3`

**Hover state (CSS group-hover, no JS):**
- Image: `group-hover:scale-[1.04]`
- Overlay: changes to `from-black/60 via-black/30` (lighter)
- Description: `group-hover:opacity-100 group-hover:translate-y-0`
- CTA: `group-hover:opacity-100 group-hover:translate-y-0` (slight extra delay via inline style)
- Thin white border top: `opacity-0 group-hover:opacity-100`

**Grid layout:**
- Mobile: `grid-cols-1` (full width, each card `min-h-[320px]`)
- Tablet md: `grid-cols-2`
- Desktop lg: `grid-cols-2` (2×2 stays the same — 4 cards in 2 columns)
- Card height: `min-h-[420px]`

**Section header:** keep the current two-column left-aligned header (eyebrow + h2 + right subtitle). No change.

**Section background:** keep `bg-[#0a0a0a]` with dot grid. No change.

**Step 1: Write the new component**

Replace the entire contents of `src/components/home/WhatWeDo.tsx` with:
...
function ServiceCard({ card, index }: { card: Card; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute inset-0 bg-black/30 opacity-100 group-hover:opacity-0 transition-opacity duration-500"
      />
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-8">
        ...
        <p
          className="text-[14px] text-white/70 leading-relaxed mb-5 opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500"
        >
          {card.body}
        </p>

        {/* CTA — hidden at rest, slides up on hover (slight extra delay) */}
        <Link
          to={card.href}
...
```bash
git add src/components/home/WhatWeDo.tsx
git commit -m "feat: redesign WhatWeDo as 2x2 photo cards with hover reveal"
```
